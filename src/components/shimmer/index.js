...
const Shimmer = ({ width, height, borderRadius = 0, marginBottom = 0, marginTop = 0 }) => {
  const [positionX, setPositionX] = useState(null);
  let viewRef = null;
  return (
    <View
      style={[styles.shimmer, { width, height, borderRadius, marginBottom, marginTop }]}
      ref={ref => (viewRef = ref)}
      onLayout={() => {
        if (viewRef) {
      ...
      {positionX !== null && (
        <Animated.View
          ...
          <LinearGradient style={{ flex: 1, width: SCREEN_WIDTH }} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} locations={LOCATIONS} colors={COLORS} />
        </Animated.View>
      )}
    </View>
  );
