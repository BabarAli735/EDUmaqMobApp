  ...
  return (
    <Container style={_styles.container}>
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          navigation.navigate(Screens.TEACHER_PROFILE);
          drawer.dispatch(DrawerActions.closeDrawer());
        }}>
        <View style={{ flexDirection: 'row', paddingTop: 0, overflow: 'hidden' }}>
          ...
          <View style={{ marginLeft: 15 }}>
            <Text numberOfLines={1} style={_styles.name}>
              {profile?.userName ?? profile?.employeeName}
            </Text>
            {/* <Text style={_styles.class}>{'STD: ' + (profile?.classCourseName ?? profile?.className) + ' ' + profile?.batchName}</Text> */}
            <Text style={_styles.role}>{'ID.: ' + profile?.employeeId}</Text>
          </View>
        </View>
      </TouchableOpacity>
      {/* </ImageBackground> */}
      <View style={{ flex: 1 }}>
        ...
        <TouchableOpacity
          style={_styles.menu}
          onPress={() => {
            navigation.navigate(Screens.TEACHER_EVENTS);
            drawer.dispatch(DrawerActions.closeDrawer());
          }}>
          <Icon icon={Icons.IC_CALENDAR} size={17} color={Colors.PRIMARY} />
          <Text style={_styles.menuText}>{Strings.Events.TITLE}</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={_styles.menu}
          onPress={() => {
            navigation.navigate(Screens.ATTENDANCE_TEACHER);
            drawer.dispatch(DrawerActions.closeDrawer());
          }}>
          <Icon icon={Icons.IC_CALENDAR} size={17} color={Colors.PRIMARY} />
          <Text style={_styles.menuText}>{Strings.AttendanceTeacher.TITLE}</Text>
        </TouchableOpacity>
      ...
      <View>
        ...
        <TouchableOpacity
          ...
          <Text style={_styles.menuText}>{Strings.Main.TEACHER_MY_PROFILE}</Text>
        </TouchableOpacity>
        <TouchableOpacity
